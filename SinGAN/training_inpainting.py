...
def train_inpainting(opt,Gs,Zs,reals,masks,NoiseAmp):
    ...
    masks = functions.creat_reals_pyramid(mask,masks,opt)
    ...
    while scale_num<opt.stop_scale+1:
        ...
        # Changer cela pour prendre en compte le masque
    ...
    for epoch in range(opt.niter):
        ...
        for j in range(opt.Dsteps):
            ...
            if (j==0) & (epoch == 0):
                if (Gs == []) & (opt.mode != 'SR_train'):
                    ...
                    RMSE = torch.sqrt(criterion((1-mask)*real, (1-mask)*z_prev))
        ...
        for j in range(opt.Gsteps):
            ...
            if alpha!=0:
                ...
                rec_loss = alpha*loss((1-mask)*netG(Z_opt.detach(),z_prev),(1-mask)*real)
...
def train_paint(opt,Gs,Zs,reals,NoiseAmp,centers,paint_inject_scale):
    ...
    while scale_num<opt.stop_scale+1:
        if scale_num!=paint_inject_scale:
            ...
            z_curr,in_s,G_curr = train_single_scale(D_curr,G_curr,reals[:scale_num+1],masks[:scale_num+1], Gs[:scale_num],Zs[:scale_num],in_s,NoiseAmp[:scale_num],opt,centers=centers)
